...
class ConvNet(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var):
        ...
        self.fc3 = nn.Linear(n_latent_var, n_latent_var*action_dim)
        self.fc4 = nn.Linear(n_latent_var*action_dim, n_latent_var)
        self.fc5 = nn.Linear(n_latent_var, action_dim)
        self.action_layer = nn.Linear(action_dim, action_dim)
        self.value_layer = nn.Linear(action_dim, 1)
    ...
    def act(self, state):
        x = self.fullyLayer(torch.from_numpy(state).float())#Add torch.tanh around it
        action_probs = torch.tanh(self.action_layer(x))
    ...
    def tester(self, state, action):
        # print(state.shape, action.shape)
        x = self.fullyLayer(torch.from_numpy(state).float())
